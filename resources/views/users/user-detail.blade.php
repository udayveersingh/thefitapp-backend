<x-layout.default>
    ...
    <div>
        ...
        <div class="pt-5">
            ...
            <div class="grid lg:grid-cols-1 grid-cols-1 gap-6 mt-2">
                <div x-data="userReferralBalance">
                    <script src="{{ asset('/assets/js/simple-datatables.js') }}"></script>
                    <div class="panel px-0 border-[#e0e6ed] dark:border-[#1b2e4b]">
                        <div class="px-5">
                            <div class="md:absolute md:top-5 ltr:md:left-5 rtl:md:right-5">
                                <div class="flex items-center gap-2 mb-5">
                                 <h5 class="font-semibold text-lg dark:text-white-light">Referral Friends List</h5>
                                </div>
                            </div>
                        </div>
                        <div class="referral-balance-table">
                            <table id="referralBalanceTable" class="whitespace-nowrap"></table>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        document.addEventListener("alpine:init", () => {
            Alpine.data("userReferralBalance", () => ({
                datatable1: null,
                init() {
                    this.datatable1 = new simpleDatatables.DataTable('#referralBalanceTable', {
                        data: {
                            headings: ['Name','Transaction Type','Transaction Date', 'Earnings'],
                            data: [
                                @foreach ($user_referral_balance as $user_referral)
                                    ['{{$user_referral->name }}',
                                    '{{$user_referral->transaction_type}}',
                                    '{{$user_referral->date}}',
                                    '{{$user_referral->credit_amount}}',
                                    ],
                                @endforeach
                            ]
                        },
