<x-layout.default>
    ...
    <div x-data="usersWithdrawlList">
        ...
        <div class="panel px-0 border-[#e0e6ed] dark:border-[#1b2e4b]">
            <div class="px-5">
                <h5 class="md:absolute md:top-[25px] md:mb-0 mb-5 font-semibold text-lg dark:text-white-light">KYC Pending Approval Requests
                </h5>
            </div>
            ...
            Alpine.data("usersWithdrawlList", () => ({
                ...
                init() {
                    this.datatable1 = new simpleDatatables.DataTable('#usersWithdrawlTable', {
                        data: {
                            headings: ['Name','Wallet Address','PAN', 'Aadhar','Requested Date','Action'],
                            data: [
                                @foreach ($kyc_pending_list as $kyc_list)
                                    ['{{ ucfirst($kyc_list->name) }}',
                                        '{{ $kyc_list->wallet_address }}',
                                        '<a target="_blank" href="{{ public_path("storage/images/").$kyc_list->user_id.$kyc_list->kyc_doc_2 }}">{{ $kyc_list->kyc_doc_1 }}</a>',
                                        '<a target="_blank" href="{{ public_path("storage/images/").$kyc_list->user_id.$kyc_list->kyc_doc_2 }}">{{ $kyc_list->kyc_doc_2 }}</a>',
                                        '{{$kyc_list->updated_at}}',
